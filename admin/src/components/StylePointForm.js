...
function StylePointForm() {
  ...
  const handleStylePointClick = async (stylePoint) => {
    try {
      const response = await fetch(`/admin/stylepoint/${stylePoint.id}`);
      if (response.ok) {
        const data = await response.json();
        setSelectedStylePoint(data.stylePoint);
        ...
        const fetchedCoordiLooks = data.stylePoint.coordiLookList;

        setBrands(fetchedBrands);
        setCoordiLooks(fetchedCoordiLooks);

        setError(null);
      } else {
        handleError('StylePoint 조회 실패', response.status);
      }
    } catch (error) {
      handleError('StylePoint 조회 오류', error.message);
    }
  };
 ...
 <h4>Brands:</h4>
              {brands && brands.map((brand) => (
                <li key={brand.id}>{brand.name || 'NULL'}</li>
              ))}
